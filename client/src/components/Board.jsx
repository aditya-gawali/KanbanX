...
const Board = ({ board, columns }) => {
    ...
    const fetchBoardDetails = async (id) => {
        try {
            const response = await axios.get(`ttp://localhost:8080/boards/${id}`)
    ...
    const handleSaveColumn = async () => {
        ...
        if (newColumnTitle) {
            ...
            const res = await axios.post("ttp://localhost:8080/columns/", {
    ...
    const handleDeleteColumn = (id) => {
        ...
        axios.delete(`ttp://localhost:8080/columns/${id}`, {
            boardId: board._id
        }).then(response => {
    ...
    const handleSaveTask = async () => {
        ...
        if (newTaskTitle) {
            try {
                ...
                await axios.post("ttp://localhost:8080/tasks/", {
                    title: newTaskTitle,
                    columnId,
                    boardId: board._id
                });
    ...
    const handleDeleteTask = async (columnId, taskId) => {
        ...
        try {
            ...
            await axios.delete(`ttp://localhost:8080/tasks/${taskId}?columnId=${columnId}`);
            toast.update(toastId, {
    ...
    const moveTask = async (taskId, newColumnId) => {
        try {
            // Update in MongoDB
            const res = await axios.put(`ttp://localhost:8080/tasks/${taskId}`, {
